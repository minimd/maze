...
class TenPrintPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double y = 0; y < size.height; y += step) {
      for (double x = 0; x < size.width; x += step) {
        ...
        final centerX = x + step / 2;
        final centerY = y + step / 2;
        final lineLength = step * 0.5;

        // Calculate the start and end points based on the current angle
        final startX = centerX - cos(angle) * lineLength;
        final startY = centerY - sin(angle) * lineLength;
  ...
  bool shouldRepaint(covariant TenPrintPainter oldDelegate) {
    // Repaint every frame because the animation value is always changing
    return true;
  }
}